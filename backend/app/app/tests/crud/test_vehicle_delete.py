...
def test_vehicle_delete_non_existing(
    db: Session
) -> None:
    """Try to delete vehicle with non-existing ID from the empty database."""
    vehicle = crud.vehicle.remove(db, id=randint(1, 10000))
    assert vehicle is None, "There is no such record in the database"


def test_vehicle_delete_with_incorrect_id(
    db: Session
) -> None:
    """Try to delete vehicle with incorrect ID."""
    for id in [-118, 0, "Hello", 3.14, ""]:
        vehicle = crud.vehicle.remove(db, id=id)  # type: ignore
        assert vehicle is None, "There is no such record in the database"


def test_vehicle_delete_correct(
    db: Session
) -> None:
    ...
    vehicle = crud.vehicle.remove(db, id=created_vehicle.id)
    ...
    assert crud.vehicle.count(db) == 0, "The database is empty"
