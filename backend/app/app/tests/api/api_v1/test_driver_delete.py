...
def test_driver_delete_non_existent(
    client: TestClient,
    db: Session
) -> None:
    """Try to delete non-existant driver from the empty database."""
...
def test_driver_delete_correct(
    ...
    """Correct removal of the driver from the database."""
    ...
    response = client.delete(f"{PATH}/{driver_in_db.id}/")
    assert response.status_code == 200, "The driver was deleted successfully"
    assert response.headers["Content-Type"] == "application/json", "Response content type"
    driver = response.json()
    assert driver["first_name"] == first_name, "The first name of the deleted driver"
