...
def test_vehicle_delete_non_existent(
    client: TestClient,
    db: Session
) -> None:
    """Try to delete non-existant vehicle from the empty database."""
...
def test_vehicle_delete_correct(
    ...
    """Correct removal of the vehicle from the database."""
    ...
    response = client.delete(f"{PATH}/{vehicle_in_db.id}/")
    assert response.status_code == 200, "The vehicle was deleted successfully"
    assert response.headers["Content-Type"] == "application/json", "Response content type"
